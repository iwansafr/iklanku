
      <div class="product ml-4 mr-4">
				<div class="card mb-3 product_box">
					<span class="badge badge-success pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">available</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Majapahit</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Vertical / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
				<div class="card mb-3 product_box">
					<span class="badge badge-danger pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">unavailable</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Prof Hamka</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Horizontal / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
			</div>
      <div class="product ml-4 mr-4">
				<div class="card mb-3 product_box">
					<span class="badge badge-success pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">available</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Majapahit</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Vertical / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
				<div class="card mb-3 product_box">
					<span class="badge badge-danger pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">unavailable</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Prof Hamka</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Horizontal / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
			</div>
      <div class="product ml-4 mr-4">
				<div class="card mb-3 product_box">
					<span class="badge badge-success pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">available</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Majapahit</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Vertical / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
				<div class="card mb-3 product_box">
					<span class="badge badge-danger pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">unavailable</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Prof Hamka</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Horizontal / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
			</div>
      <div class="product ml-4 mr-4">
				<div class="card mb-3 product_box">
					<span class="badge badge-success pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">available</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Majapahit</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Vertical / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
				<div class="card mb-3 product_box">
					<span class="badge badge-danger pull-right" style="width: 20vw;padding-top: 1vw; position: absolute;top: 2vw;right: 10px;font-size: 3vw;">unavailable</span>
					<a href="<?php echo base_url('home/detail') ?>" ><img style="border-top-right-radius: 10%;border-top-left-radius: 10%;" src="<?php echo base_url('templates/iklanku/');?>img/map.jpg" class="card-img-top" alt="..."></a>
				  <div class="card-body">
				  	<div class="row">
					    <div class="col">
					    	<h5 class="card-title" style="font-size: 4vw;">Kota Semarang</h5>
					    </div>
					    <div class="col-7 description_product pl-0">
					    	<p style="font-size: 3vw;margin-bottom: -1vw;">Jl. Prof Hamka</p>
					    	<p class="card-text">
					    		<small style="font-size: 2vw;" class="text-muted">
					    			Billboard / 5x10M / Horizontal / FL
					    		</small>
					    	</p>
					    </div>
				  	</div>
				  </div>
				</div>
			</div>
  